...
#extracting experience(syntex from-month to to-month eg. 20-01-2018 to 20-01-2019)
...
#calculating number of months between two dates(experience month calculation) using datetime module
...
#extract email(syntex @.) using regular expression module
...
#mobile number extract using regular expression
...
#extracting skills based on the data set present in skills files
...
def extract_education(nlp_text, noun_chunks):
    '''
    ...
    tokens = [token.text for token in nlp_text if not token.is_stop]
    tokens = " ".join(tokens)
    data = pd.read_csv(os.path.join(os.path.dirname(__file__), 'education.csv'))
    educations = list(data.columns.values)
    education = []
    #check for one-grams
    for edu in educations:
        if(re.search(r'{0}'.format(edu), tokens)):
           education.append(edu)
            
    return ", ".join(education)#[i.capitalize() for i in set([i.lower() for i in education])]

def extract_designation(nlp_text):
    tokens = [token.text for token in nlp_text if not token.is_stop]
    tokens = " ".join(tokens)
    data = pd.read_csv(os.path.join(os.path.dirname(__file__), 'designation.csv'))
    designations = list(data.columns.values)
    designation = []
    #check for one-grams
    for des in designations:
        if(re.search(r'{0}'.format(des), tokens)):
           designation.append(des)
            
    return ", ".join(designation)#[i.capitalize() for i in set([i.lower() for i in education])]
